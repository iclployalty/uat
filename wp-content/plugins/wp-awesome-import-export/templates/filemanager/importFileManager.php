...
<table class='widefat filemanagerdata'>
    ...
    <tbody>
        <?php foreach($importFileManager as $value){ ?>
        <tr class="<?php echo $value["file_id"]; ?>">
            <td><?php echo $value["file_name"]; ?></td>
            <td>
                <span class="<?php echo strtolower($value["file_type"]); ?>"><?php echo strtolower($value["file_type"]); ?></span>
            </td>
            ...
            <td>
                ...
                <a class="deleteAllFileRecords" fileId='<?php echo $value["file_id"]; ?>' href="javascript:void(0)" title="delete all records"><i class="fa fa-database"></i></a>
            </td>
        </tr>
        <?php } ?>
    </tbody>
