...
// One Click Import demo functions
	...
	class Rehub_Admin{
		...
		function rehub_admin_menu(){
			...
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				//$required_plugins = add_submenu_page( 'rehub', __( 'Required plugins', 'rehub_framework' ), __( 'Required plugins', 'rehub_framework' ), 'administrator', 'rehub-install-plugins', array( $this, 'rehub_plugins_sub' ) );
				$demo_content = add_submenu_page( 'rehub', __( 'Demo content', 'rehub_framework' ), __( 'Demo Import', 'rehub_framework' ), 'administrator', 'import_demo', array( $this, 'demo_content_sub' ));
				$demos = add_submenu_page( 'rehub', __( 'Full demo stacks', 'rehub_framework' ), __( 'Demo Site Clones', 'rehub_framework' ), 'administrator', 'rehub-demos', array( $this, 'rehub_demos_tab' ) );				
		...
		function demo_content_sub(){
			if ( !rh_is_plugin_active( 'one-click-demo-import/one-click-demo-import.php' ) ) { ?>
			<h2></h2>
		   <div class="notice notice-info"><p><?php printf(__( 'Please, install and activate One Click Demo Import plugin <a href="%s">here</a>', 'rehub_framework' ), admin_url( 'admin.php?page=rehub-plugins' )); ?></p></div>
			<?php
			} 
		}
