...
<?php if(rehub_option('rehub_sidebar_left') =='1') : ?>
	...
	.sidebar{float: left}
<?php endif; ?>
<?php if (rehub_option('fullfooter_color_background') !='') :?>
	#footerfullarea{background-color: <?php echo rehub_option('fullfooter_color_background'); ?> }
<?php endif; ?>	
...
.rehub-main-color-border, .rh-big-tabs-li.active a, .rh-big-tabs-li:hover a, nav.top_menu > ul > li.vertical-menu.border-main-color .sub-menu{border-color: <?php echo $maincolor; ?>;}
...
input#ywqa-send-answer{ background: <?php echo $seccolor ?> !important; color: #fff !important; outline: 0}
...
nav.top_menu > ul > li.vertical-menu.border-sec-color > .sub-menu{border-color: <?php echo $seccolor ?>}
...
<?php 	
	if (REHUB_NAME_ACTIVE_THEME == 'REWISE' || rehub_option('enable_smooth_btn') == 1):?>
		...
		.price_count, .rehub_offer_coupon, #buddypress .dir-search input[type=text], .gmw-form-wrapper input[type=text], .gmw-form-wrapper select, .rh_post_layout_big_offer .priced_block .btn_offer_block, #buddypress a.button, .btn_more{border-radius: 100px}
