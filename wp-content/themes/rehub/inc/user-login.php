					...
					<div class="rehub-register-popup">	 
						...
						<form id="rehub_registration_form_modal" action="<?php echo home_url( '/' ); ?>" method="POST">
							...
							if($show_terms_page){ ?>
								<div class="re-form-group mb20">
									<div class="checkbox">
										<label><input name="rehub_terms" type="checkbox"> <?php echo sprintf( __( 'I accept the <a target="_blank" href="%s">Terms & Conditions</a>', 'rehub_framework' ), is_numeric($show_terms_page) ? get_the_permalink($show_terms_page) : esc_url($show_terms_page) ) ?></label>
									</div>
								</div><?php
							}
							...
							if($show_policy_page){ ?>
								<div class="re-form-group mb20">
									<div class="checkbox">
										<label><input name="rehub_policy" type="checkbox"> <?php echo sprintf( __( 'I accept the <a target="_blank" href="%s">Privacy Policy</a>', 'rehub_framework' ), is_numeric($show_policy_page) ? get_the_permalink($show_policy_page) : esc_url($show_policy_page) ) ?></label>
									</div>
								</div><?php
							}							
...
function rehub_register_member_popup_function(){
	...
	if( !check_ajax_referer( 'ajax-login-nonce', 'register-security', false) ){
 	...
 	elseif($show_policy_page and !isset($_POST['rehub_policy'])){
		echo json_encode(array('error' => true, 'message'=> '<div class="wpsm_box warning_type">'.__('Please accept the Privacy Policy before registering', 'rehub_framework').'</div>'));
		die();
 	} 	
