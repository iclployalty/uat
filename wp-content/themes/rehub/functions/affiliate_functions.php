	        ...
	        <div class="priced_block clearfix <?php echo $reveal_enabled; echo $coupon_style; ?> <?php echo esc_html($wrapperclass);?>">
		    	...
		    	<?php if ($coupon_mask_enabled =='1') :?>
		    		<?php if($showme !='price') : ?>
			    		<div class="post_offer_anons">
			    			<?php wp_enqueue_script('zeroclipboard'); ?>
		                	<span class="coupon_btn re_track_btn btn_offer_block rehub_offer_coupon masked_coupon <?php if(!empty($offer_coupon_date)) {echo $coupon_style ;} ?>" data-clipboard-text="<?php echo esc_html ($offer_coupon) ?>" data-codeid="<?php echo $post->ID?>" data-dest="<?php echo esc_url($offer_url) ?>">
		                		<?php if($offer_btn_text !='') :?>
			            			<?php echo esc_html ($offer_btn_text) ; ?>
		    	...
		    	<?php else : ?>
					<?php if(!empty($offer_coupon) && $showme !='price' && $coupon =='1') : ?>
						<?php wp_enqueue_script('zeroclipboard'); ?>
					  	<div class="rehub_offer_coupon not_masked_coupon <?php if(!empty($offer_coupon_date)) {echo $coupon_style ;} ?>" data-clipboard-text="<?php echo $offer_coupon ?>"><span class="coupon_text"><?php echo $offer_coupon ?></span> <i class="fal fa-cut fa-rotate-180"></i>
					  	</div>
				  	<?php endif;?>		    		
		        <?php endif; ?>	            	        
	        </div>
...
if ($post->post_type == 'product'){
	$offer_url_exist = get_post_meta( $post->ID, '_product_url', true );
}
else{
	$offer_url_exist = get_post_meta( $post->ID, 'rehub_offer_product_url', true );
}
    	...
    	if (!empty($cegg_field_array) && is_array($cegg_field_array)) {
			...
			if (!empty($cegg_fields) && is_array($cegg_fields)) {
				...
				if ('product' == get_post_type($post_id)) {
		    		...
		    		if ($domain){		    			
		    			if(rehub_option('save_store_ce') == 1 && rehub_option('enable_brand_taxonomy') == 1 && 'post' == $post->post_type){
...
if (rehub_option('rh_front_reviewform_id') !='') {
	...
	$formidforreviews = array_map( 'trim', explode( ",", $formidforreview ) );
	foreach ($formidforreviews as $formidforreview) {
