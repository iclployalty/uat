...
if (defined('wcv_plugin_dir')) {	
	if ( class_exists( 'WCVendors_Pro' ) ) {
		...
		add_action( 'wcv_pro_store_settings_saved', 'rh_wcv_gwm_save_location' );
	}
	else{
	...
	function rh_wcv_gwm_save_location( $vendor_id ){
		if( !function_exists('gmw_update_user_location') ) 
			return;
		
		$address = array();
		$address['street'] = ( isset( $_POST[ '_wcv_store_address1' ] ) ) ? sanitize_text_field( $_POST[ '_wcv_store_address1' ] ) : '';
		$address['apt'] = ( isset( $_POST[ '_wcv_store_address2' ] ) ) ? sanitize_text_field( $_POST[ '_wcv_store_address2' ] ) : '';
		$address['city'] = ( isset( $_POST[ '_wcv_store_city' ] ) ) ? sanitize_text_field( $_POST[ '_wcv_store_city' ] ) : '';
		$address['state'] = ( isset( $_POST[ '_wcv_store_state' ] ) ) ? sanitize_text_field( $_POST[ '_wcv_store_state' ] ) : '';
		$address['country'] = ( isset( $_POST[ '_wcv_store_country' ] ) ) ? sanitize_text_field( $_POST[ '_wcv_store_country' ] ) : '';
		$address['zipcode'] = ( isset( $_POST[ '_wcv_store_postcode' ] ) ) ? sanitize_text_field( $_POST[ '_wcv_store_postcode' ] ) : '';
...
if(get_option( 'woocommerce_enable_reviews' ) === 'yes' && rehub_option('woo_helpornot')){
	...
	if ( !wp_is_post_revision( $post_id ) ) {
		...
		if (!empty($thecriteria))  {
		    foreach ($thecriteria as $criteria) {
		    	$score += (int)$criteria['review_post_score']; $total_counter ++;
		    }
		}
	    if ($manual_score)  {
