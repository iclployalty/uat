...
define('RH_MAIN_THEME_VERSION', '7.6.8');
...
function rehub_framework_register_scripts() {
	...
	wp_register_style('rehubfontawesome', get_template_directory_uri() . '/vafpress-framework/public/css/vendor/font-awesome.min.css', array(), '5.0.9');
...
function rehub_enqueue_scripts() {
	...
	if( is_singular() && (rehub_option('enable_btn_userreview') == '1' || rehub_option('woo_helpornot') == '1')){
		wp_enqueue_script( 'commentplus_re', get_template_directory_uri().'/js/commentplus_re.js', array('jquery'), '1.0', 1 );
		wp_localize_script( 'commentplus_re', 'cplus_var', array('url' => admin_url('admin-ajax.php'),'nonce' => wp_create_nonce('commre-nonce')));
	}	
}
}
...
function rh_post_header_meta ($admin_exist = true, $time_exist = true, $view_exist = true, $comment_exist = true, $cats_post = true ){
	...
	if(rehub_option('exclude_comments_meta') != 1 && ($comment_exist != false)){ ?>
		<?php if($comment_exist=='compact'):?>
			<span class="comm_count_meta"><?php comments_popup_link( __('0','rehub_framework'), __('1','rehub_framework'), __('%','rehub_framework'), 'comm_meta', ''); ?></span>
...
/*function rehub_admin_error_notice() {
    ...
    if ( ! get_user_meta($user_id, 'ignore_notices_rehub76') ) {
		if ($page=='rehub' || $page=='rehub-support' || $page=='rehub-plugins' || $page=='rehub-demos' || $page=='vpt_option' ) {
			$class = "error";
			$message = 'Some important changes in <a href="https://sizamtheme.support-hub.io/articles/rehub-76-major-update" target="_blank">Rehub 7.6 version</a> which can affect your site.' ;
...
function rehub_nag_ignore() {
    ...
    if ( isset($_GET['rehub_nag_ignore']) && '0' == $_GET['rehub_nag_ignore'] ) {
        add_user_meta($user_id, 'ignore_notices_rehub76', 'true', true);
